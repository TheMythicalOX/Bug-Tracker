...
const CreateTicket = async (ticket: Ticket) => {
  let error = await axios
    .get(
      `https://${process.env.REACT_APP_SERVER_IP}/ticketcreate.php/?name=${ticket.title}`,
  ...
  error = await axios
    .post(
      `https://${process.env.REACT_APP_SERVER_IP}/ticketcreate.php`,
      ticket,
      { withCredentials: true }
    )
    .then((res) => {
  ...
  error = await axios
    .post(
      `https://${process.env.REACT_APP_SERVER_IP}/ticketassign.php`,
      ticket,
      { withCredentials: true }
    )
    .then((res) => {
